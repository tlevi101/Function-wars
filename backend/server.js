...
io.use(function(socket, next){
  ...
  .on('connection', (socket) =>{
	socket.on('join chat rooms', async () => {
		...
		if(!user) return;
		const friendShips = await Friendship.findAll({where: { [Op.or]: [{user_id: user.id}, {friend_id: user.id}] }});
		friendShips.forEach(friendShip => {
			socket.join(`chat-${friendShip.id}`);
			socket.emit(`joined chat room`, {room: `chat-${friendShip.id}`, friend_id: friendShip.friend_id === user.id ? friendShip.user_id : friendShip.friend_id});
		});
	});
