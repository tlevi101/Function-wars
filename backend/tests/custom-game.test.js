...
describe('test custom game through API  and socket', () =>{
    ...
    describe('create custom game',()=> {
        ...
        beforeAll(async() =>{
            ...
            friendSockets = serverSockets.map((socket, index) =>{
        ...
        test('friends gets the invite', (done) => {
            ...
            userClients[0].emit('create custom game', {fieldID:field.id, isPrivate:true, friendIDs:friends.map((friend)=>friend.id)})
        })
    ...
    describe('test join to public custom game',() =>{
        ...
        test('join to custom with the correct UUID',(done)=>{
            userClients[1].on('waiting room joined', async ()=>{
                const response = await request.get(`/wait-rooms/${UUID}`)
                    .set('Authorization', `Bearer ${userTokens[1]}`);
                expect(response.status).toBe(200);
                expect(response.body.waitRoom).toEqual(expect.objectContaining({
                    roomUUID: UUID,
    ...
    describe('test join to private custom game',() =>{
        ...
        beforeAll(async ()=>{
            ...
            nonFriendSockets = serverSockets.map((socket, index) =>{
    ...
    describe('test disconnections',() =>{
        ...
        test('owner disconnected from server', (done)=>{
            userClients[1].on('wait room owner left', ()=>{
        ...
        test('owner disconnected and reconnected', (done)=>{
            userClients[1].on('waiting room joined', async ()=>{
                userClients[0].disconnect();
                setTimeout(async () =>{
                    userClients[0] = new Client('http://localhost:3000', {
