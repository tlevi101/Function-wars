...
describe('FriendsController API tests when a guest making the request', () => {
    ...
    test('GET /friends - guest cannot make request', async () => {
        const response = await request.get('/friends').set('Authorization', `Bearer ${guestToken}`);
    ...
    test('GET /friends/online - guest cannot make request', async () => {
        const response = await request.get('/friends/online').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
    ...
    test('GET /friends/requests - guest cannot make request', async () => {
        const response = await request.get('/friends/requests').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
    ...
    test('GET /friends/:id/chat - guest cannot make request', async () => {
        const response = await request.get('/friends/1/chat').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
    ...
    test('PUT /friends/requests/:id/accept - guest cannot make request', async () => {
        const response = await request.put('/friends/requests/1/accept').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
    ...
    test('DELETE /friends/:id - guest cannot make request', async () => {
        const response = await request.delete('/friends/1').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
    ...
    test('POST /friends/:id - guest cannot make request', async () => {
        const response = await request.post('/friends/1').set('Authorization', `Bearer ${guestToken}`);
        expect(response.status).toBe(403);
        expect(response.body).toEqual(
            expect.objectContaining({
                message: 'Guest cannot make this request!',
...
describe('FriendsController API tests on GET /friends/:id/chat remaining failure cases', () => {
    ...
    test('friend not found', async () => {
        const response = await request.get(`/friends/${100}/chat`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(404);
    ...
    test('friendship not found', async () => {
        const response = await request.get(`/friends/${5}/chat`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(404);
...
describe('FriendsController API tests on DELETE /friends/:id remaining failure cases', () => {
    ...
    test('DELETE /friends/:id friend not found', async () => {
        const response = await request.delete(`/friends/${100}`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(404);
    ...
    test('DELETE /friends/:id User is not part of the friendship', async () => {
        const response = await request.delete(`/friends/${7}`).set('Authorization', `Bearer ${userTokens[0]}`);
    ...
    test('DELETE /friends/:id User wants to delete pending friendship', async () => {
        const response = await request.delete(`/friends/${2}`).set('Authorization', `Bearer ${userTokens[0]}`);
...
describe('FriendsController API tests on POST /friends/:id (add friend) remaining failure cases', () => {
    ...
    test('POST /friends/:id other user not found', async () => {
        const response = await request.post(`/friends/${100}`).set('Authorization', `Bearer ${userTokens[0]}`);
    ...
    test('POST /friends/:id other user tried to add himself', async () => {
        const response = await request.post(`/friends/${1}`).set('Authorization', `Bearer ${userTokens[0]}`);
...
describe('FriendsController API tests on success', () => {
    ...
    test('GET /friends - get friends', async () => {
        const response = await request.get('/friends').set('Authorization', `Bearer ${userTokens[0]}`);
    ...
    test('GET /friends/online - get online friends', async () => {
        const response = await request.get('/friends/online').set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(200);
        expect(response.body).toEqual({
            friends: expect.arrayContaining([]),
        });
    ...
    test('GET /friends/requests - get friend requests', async () => {
        const response = await request.get('/friends/requests').set('Authorization', `Bearer ${userTokens[1]}`);
        ...
        const response2 = await request.get('/friends/requests').set('Authorization', `Bearer ${userTokens[4]}`);
    ...
    test('GET /friends/:id/chat - get friend chat', async () => {
        const response = await request.get(`/friends/${3}/chat`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(200);
    ...
    test('GET /friends/:id/chat - get friend chat empty', async () => {
        const response = await request.get(`/friends/${4}/chat`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(200);
    ...
    test('DELETE /friends/:id - delete friend', async () => {
        const response = await request.delete(`/friends/${3}`).set('Authorization', `Bearer ${userTokens[0]}`);
        expect(response.status).toBe(200);
    ...
    test('POST /friends/:id - send friend request', async () => {
        const response = await request.post(`/friends/${3}`).set('Authorization', `Bearer ${userTokens[1]}`);
        expect(response.status).toBe(200);
    ...
    test('GET /friends/requests- request received', async () => {
        const response = await request.get(`/friends/requests`).set('Authorization', `Bearer ${userTokens[2]}`);
        expect(response.status).toBe(200);
