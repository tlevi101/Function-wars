...
describe('stress tests', () => {
	...
	test('stress', async () => {
		...
		await Promise.all(serverSockets.map((serverSocket, index) => {
		...
		stressTestUserClients.forEach((stressUserClient, index) => {
			...
			const clientSidePromise = new Promise((resolve) => {
				...
				stressUserClient.on('joined game', async ({room, players, field}) => {
					...
					expect(RuntimeMaps.games.get(room).players.some(player => player.ID == stressTestUsers[index].id)).toBe(true);
					expect(RuntimeMaps.groupChats.has('chat-'+room)).toBe(true);
					expect(RuntimeMaps.groupChats.get('chat-'+room).Users.some(player => player.id == stressTestUsers[index].id)).toBe(true);
					resolve();
				});
			});
