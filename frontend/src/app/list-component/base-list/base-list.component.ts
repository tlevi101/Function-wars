import { Component, ComponentRef, ElementRef, EventEmitter, Input, Output, QueryList, ViewChild, ViewChildren } from '@angular/core';
import { Router } from '@angular/router';
import { ConfirmWithInputComponent, ConfirmWithInputData } from 'src/app/pop-up/confirm-with-input/confirm-with-input.component';
...
export class BaseListComponent {
	...
	@Output() actionClickedEvent: EventEmitter<{ action: Action, data: any[], confirmInput?: string }> = new EventEmitter();
	...
	actionClicked(
		action: Action,
		dataId: number | undefined = undefined
	): void {
		...
		if (singularData && action.confirmRequired(singularData)) {
			if (action.confirmType === ConfirmType.DEFAULT && action.confirmData?.myName === 'ConfirmData') {
				...
				console.log(this.selectedDataForAction);
				this.confirm.show(action.confirmData);
			}
			else if (action.confirmType === ConfirmType.WITH_INPUT && action.confirmData?.myName === 'ConfirmWithInputData') {
				this.selectedDataForAction = [this.data.find((data: any) => data.id === dataId)];
				this.confirmWithInput.show(action.confirmData);
			}
		}
		else if(action.confirmRequired() && !dataId){
			this.selectedDataForAction = this.checkBoxes.filter((checkBox: ElementRef) => checkBox.nativeElement.checked).map((checkBox: ElementRef) => {
			...
			this.selectedDataForAction  = this.CheckBoxes.filter((checkBox: ElementRef) => checkBox.nativeElement.checked)
	...
	handleConfirmEvent($event:any | undefined = undefined): void {
		...
		this.actionClickedEvent.emit({ action: this.lastAction, data: this.selectedDataForAction, confirmInput });
	}
	...
	selectAll(event: any) {
		this.checkBoxes.forEach((checkBox: ElementRef) => {
			...
			console.log(checkBox.nativeElement);
		});
	}
