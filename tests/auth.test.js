...
describe('POST /', () => {
	...
	it('POST /register password is too short', () => {
		return request(app)
			...
			.then((res) => {
				expect(res.body).toEqual(
					expect.objectContaining({
						error: expect.stringContaining('Validation error: Password must be between 8 and 20 characters'),
					})
				  );
			});
	});
	...
	it('POST /register password is too long', () => {
		return request(app)
			.post('/register')
			.send({ password: 'aaaaaaaaaaaaaaaaaaaaa', passwordAgain: 'aaaaaaaaaaaaaaaaaaaaa' })
			.expect('Content-Type', /json/)
			.expect(400)
			.then((res) => {
				expect(res.body).toEqual(
	...
	it('POST /register name is too short', () => {
		return request(app)
			...
			.then((res) => {
				expect(res.body).toEqual(
					expect.objectContaining({
						error: expect.stringContaining('Validation error: Username must be between 3 and 20 characters'),
					})
				  );
			});
	});
	...
	it('PUT /password-reset/:uuid password too short', async () => {
		...
		return request(app)
			.put('/password-reset/'+uuid)
			.send({password:'123456781234567812345678', passwordAgain: '123456781234567812345678'})
			.expect('Content-Type', /json/)
			.expect(400)
			.then((res) => {
				expect(res.body).toEqual(
